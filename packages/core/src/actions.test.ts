...
describe("placeSnack", () => {
  ...
  test("should throw INVALID_SNACK_PLACEMENT when fox places in tanuki half (rows 3-4)", () => {
    const fn = () => placeSnack(baseState, "fox", [3, 2]);
    expect(fn).toThrow(
      expect.objectContaining({ code: ErrorCode.INVALID_SNACK_PLACEMENT }),
    );
  });

  test("should throw INVALID_SNACK_PLACEMENT when tanuki places in fox half (rows 0-1)", () => {
    const fn = () => placeSnack(baseState, "tanuki", [0, 2]);
    expect(fn).toThrow(
      expect.objectContaining({ code: ErrorCode.INVALID_SNACK_PLACEMENT }),
    );
  });
  ...
  test("should throw INVALID_SNACK_PLACEMENT if player already placed", () => {
    const state = { ...baseState, foxSnack: [1, 1] as [number, number] };
    const fn = () => placeSnack(state, "fox", [1, 2]);
    expect(fn).toThrow(
      expect.objectContaining({ code: ErrorCode.INVALID_SNACK_PLACEMENT }),
    );
  });
  ...
  test("should set foxSnack and stay in hide-snack when only fox has placed", () => {
    const newState = placeSnack(baseState, "fox", [1, 1]);
    expect(newState.foxSnack).toEqual([1, 1]);
  ...
  test("should set tanukiSnack and stay in hide-snack when only tanuki has placed", () => {
    const newState = placeSnack(baseState, "tanuki", [3, 1]);
    expect(newState.tanukiSnack).toEqual([3, 1]);
    expect(newState.foxSnack).toBeNull();
    expect(newState.phase).toBe("hide-snack");
  });

  test("should transition to select-card once both players have placed", () => {
    const afterFox = placeSnack(baseState, "fox", [1, 1]);
    const afterBoth = placeSnack(afterFox, "tanuki", [3, 1]);
    expect(afterBoth.foxSnack).toEqual([1, 1]);
    expect(afterBoth.tanukiSnack).toEqual([3, 1]);
  ...
  test("should transition to select-card regardless of placement order", () => {
    const afterTanuki = placeSnack(baseState, "tanuki", [3, 2]);
    const afterBoth = placeSnack(afterTanuki, "fox", [1, 2]);
    expect(afterBoth.phase).toBe("select-card");
    expect(afterBoth.foxSnack).toEqual([1, 2]);
    expect(afterBoth.tanukiSnack).toEqual([3, 2]);
  });
});
